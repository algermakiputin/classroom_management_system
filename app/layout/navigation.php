<div>
	<nav class="navi">
		<div class="container">
			<div class="row">
				<ul class="nav-btn">
					<a href="/app/dashboard.php">
						<li>
							<i class="fa fa-tachometer" aria-hidden="true"></i> Dashboard
						</li>
					</a>
					<a href="/app/classroom/room.php">
						<li>
							<i class="fa fa-door" aria-hidden="true"></i>
							<i class="fa fa-university" aria-hidden="true"></i> Classrooms
						</li>
					</a>
					<a href="/app/schedule/set_schedule.php">
						<li>
							<i class="fa fa-clock-o"></i> Set Schedule
						</li>
					</a>
					<a href="/app/schedule/schedule.php">
						<li>
							<i class="fa fa-calendar" aria-hidden="true"></i> Schedule
						</li>
					</a>
					<a href="/app/account/account.php">
						<li>
							<i class="fa fa-user-circle-o" aria-hidden="true"></i> Accounts
						</li>
					</a>
					<a href="/app/setting.php">
						<li>
							<i class="fa fa-cog" aria-hidden="true"></i> Settings
						</li>
					</a>
					<a href="/app/lagout.php">
						<li>
							Sign Out
						</li>
					</a>
				</ul>
			</div>
		</div>
	</nav>
</div>